@if (isset($item['type']) && $item['type'] === 'header')
    <li>
        <span class="{{ $item['css_class'] ?? 'dropdown-header' }}">{{ $item['title'] ?? '' }}</span>
    </li>
        ...
        <ul class="{{ $item['dropdown_class'] ?? 'dropdown-menu' }}"
            ...
            @if(isset($item['children']) && is_array($item['children']))
                @foreach ($item['children'] as $subItem)
                    @include('jiny-service::partials.navs.horizontal.menu-item', ['item' => $subItem])
                @endforeach
            @endif
        </ul>
    </li>
...
@else
    ...
    <li class="{{ $item['container_class'] ?? 'nav-item' }}">
