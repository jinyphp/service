...
namespace Jiny\Service\Http\Controllers\Admin\Services\Categories;
...
class DestroyController extends Controller
{
    ...
    public function __invoke(Request $request, $id)
    {
        ...
        if ($hasServices) {
            return redirect()
                ->route($this->config['redirect_route'])
                ->with('error', '이 카테고리에 속한 서비스가 있어 삭제할 수 없습니다. 먼저 서비스들의 카테고리를 변경해 주세요.');
        }
        ...
        return redirect()
            ->route($this->config['redirect_route'])
            ->with('success', 'Service Category가 성공적으로 삭제되었습니다.');
    }
}